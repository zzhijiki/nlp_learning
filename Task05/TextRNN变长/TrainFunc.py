...
class TrainFunc:
    ...
    def valid_func(self):
        ...
        self.model.eval()
        with torch.no_grad():
            for batch_idx, (data, length, label) in enumerate(iter(self.valid_iter)):
                batch_x_pack = pack_padded_sequence(data, length, batch_first=True)
                batch_x_pack = batch_x_pack.cuda()
                label = label.cuda()
                # batchsize = data.shape[0]
    ...
    def predict(self):
        ...
        ans_box = []
        with torch.no_grad():
            for batch_idx, (data, length) in enumerate(iter(self.test_iter)):
                batch_x_pack = pack_padded_sequence(data, length, batch_first=True)
                batch_x_pack = batch_x_pack.cuda()

                output = self.best_model(batch_x_pack)
                pred = output.argmax(1)
